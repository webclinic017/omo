...
class MarketsController extends AppController
{
    ...
    public $helpers = array('Js' => array('Jquery'),'AssetCompress.AssetCompress');
...
//pr($lastTradeInfo);
  //          exit;
    ...
    public function see_all_shares($type='gain_by_percentage')
    {
...
$this->set('table_row_display',5);
    ...
    public function search_result()
    {
       // Configure::write('debug', 2);
        ...
        $lastTradeInfo = $this->_prepareLastTradeInfo();
        $yesterdayLastTradeInfo = $this->_prepareLastTradeInfo(0);
        $metaKey=array("name","category","logo_small");
         ...
         *
         * */
        $resultWithMeta=array();
        foreach($result as $row)
        {
            $instrument_id=$row['instrument_id'];
            $logo=$fundamentaInfo[$instrument_id]['logo_small']['meta_value'];
            $instrument_name=$fundamentaInfo[$instrument_id]['name']['meta_value'];
            $category=$fundamentaInfo[$instrument_id]['category']['meta_value'];
            $row['logo']=$logo;
            $row['name']=$instrument_name;
            $row['category']=$category;
            $resultWithMeta[]=$row;
        }
     //   pr($result);
     //   pr($fundamentaInfo);
        echo json_encode($resultWithMeta);
        exit;
        //
...
//pr($newArr);
    ...
    public function market_frame()
    {
    //    Configure::write('debug', 2);
        $StockBangladesh = $this->Components->load('StockBangladesh');
        $sectorWiseInstrumentList = $StockBangladesh->instrumentList(1);
        $instrumentList = $StockBangladesh->instrumentList(3);
        $instrumentListIdToSector = $StockBangladesh->instrumentList(4);
       // pr($instrumentList);
        $marketId = $StockBangladesh->getMarketInfo(0);
     ...
     //   pr($lastTradeInfo);
        ...
        $frame_base="total_value";
        $sortedSymbolArr = array();
        $sortedCodeArr = array();
        $sortedSectorArr = array();
        $market_turnover = 0;

        foreach($lastTradeInfo as $instrument_id=>$details)
        {
            $lastTrade=array_values($details);
            $code=$instrumentList[$instrument_id];
            $ltp=$lastTrade[0]['close_price'] != 0 ?  $lastTrade[0]['close_price'] : ( $lastTrade[0]['pub_last_traded_price'] != 0 ?  $lastTrade[0]['pub_last_traded_price'] :  $lastTrade[0]['spot_last_traded_price']);
            $yday_close_price=$lastTrade[0]['yday_close_price'];
            if(!$yday_close_price) //  $yday_close_price==0 is index. so we are skipping index
                continue;
            $price_change=$ltp-$yday_close_price;
            $price_change_per=($price_change/$yday_close_price)*100;




           // pr($price_change_per);
            $lastTrade[0]['close']=$ltp;
            $lastTrade[0]['price_change']=$price_change;
            $lastTrade[0]['price_change_per']=round($price_change_per, 2);
            $sector=$instrumentListIdToSector[$instrument_id];

          //  pr($sector);
         //   pr($lastTrade);
            if ($lastTrade[0]['total_volume'] > 0) {


                $sortedSymbolArr [$sector][] = $code;
                if(isset($sortedSectorArr[$sector])) {
                    $sortedSectorArr[$sector] += $lastTrade[0][$frame_base];
                }
                else
                {
                    $sortedSectorArr[$sector]=0;
                }
                $market_turnover += $lastTrade[0][$frame_base];
                $sortedCodeArr[$code] = $lastTrade[0];
     ...
     //   pr($sortedSymbolArr);
      //  pr($market_turnover);
        ...
        $this->set("sortedSymbolArr", $sortedSymbolArr);
        $this->set("sortedCodeArr", $sortedCodeArr);
        $this->set("sortedSectorArr", $sortedSectorArr);
        $this->set("market_turnover", $market_turnover);
        $this->set("frame_base", $frame_base);
