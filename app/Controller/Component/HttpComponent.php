...
class HttpComponent extends Component
{
    ...
    */
    function getFromUrl($url, $user="", $pwd="", $verb="GET") {
        ...
        if (!$sock = @fsockopen("www.dsebd.org", $port, $errno, $errstr, $this->connect_timeout))
        {
    ...
    function table_into_array($rawHTML,$needle="",$needle_within=0,$allowed_tags="") {
        ...
        while ($tmp = strpos(strtoupper($table),"<TR") !== false) {
            ...
            while ($tmp = strpos(strtoupper($row),"<TD") !== false) {
                ...
                $string=trim(strip_tags(substr($row,$tmp,$tmp2-$tmp),$allowed_tags));
                $aryData[$rowIdx][] =str_replace(',','', $string); 
                $row = substr($row,$tmp2+5);
            }
    ...
    */
    ...
    function get_web_page($url)
    {
        $options = array(
            ...
            CURLOPT_REFERER => $url, // set referer on redirect
