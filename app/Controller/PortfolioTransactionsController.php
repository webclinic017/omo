...
class PortfolioTransactionsController extends AppController
{
    ...
    public function transaction_stat()
    {
         $this->layout = 'ajax';
        ...
        if($this->Auth->user('id')) {
            ...
            // making irn similar to official tradeins file
            if($broker_id==6) { //Sharp
                $internal_ref_no = trim($internal_ref_no);
                $internal_ref_no = str_pad($internal_ref_no, 5, "0", STR_PAD_LEFT);
                $internal_ref_no = strtoupper($internal_ref_no);
            }
            if($broker_id==11) {  // preparing inr according to apex format (padding leading zero)
                $internal_ref_no = trim($internal_ref_no);
                $internal_ref_no = str_pad($internal_ref_no, 5, "0", STR_PAD_LEFT);
                $internal_ref_no = strtoupper($internal_ref_no);
            }
            if($broker_id==5) {  // preparing inr according to Hac format (padding leading zero)
                $internal_ref_no = trim($internal_ref_no);
                $internal_ref_no = str_pad($internal_ref_no, 5, "0", STR_PAD_LEFT);
                $internal_ref_no = strtoupper($internal_ref_no);
            }
        ...
        $RawTradin = ClassRegistry::init('RawTradin');

        $allTransactions = $RawTradin->find('all',array(
                'conditions' => "RawTradin.client_ac LIKE '$internal_ref_no'",
                'order' => 'RawTradin.execute_time desc',
                'recursive' => -1
            )
        ...
        $allTransactions=Hash::extract($allTransactions, '{n}.RawTradin');


        App::uses('CakeTime', 'Utility');

        $transArr=array();
        $tableHead=array(' # ','Code','Type','Quantity','Price','Order id','Execution_id','   Transaction time  ');
        $id=0;
        foreach($allTransactions as $row)
        {
            ...
            $temp['instrumnet_code'] = $row['symbol'];
            $temp['type']=$row['side'];
            $temp['amount']=$row['execute_qty'];
            $temp['rate']=$row['execute_price'];
            $temp['dse_order_id']=$row['order_id'];
            $temp['dse_execution_id']=$row['execution_id'];
            $temp['transaction_time']=CakeTime::nice($row['execute_time']);
            $transArr[]=$temp;
            $id++;
        }
      //  $transArr[]=array();
        $this->set('allTransactions',$transArr);
        $this->set('tableHead',$tableHead);

        $this->set('pageTitleMain', 'Transaction history');
        $this->set('pageTitleSmall', 'Buy/sell transactions');
