...
$(document).ready(function() {
    ...
    //added by sb to change the chart type
     $.each(['line_{$renderTo}', 'column_{$renderTo}', 'spline_{$renderTo}', 'area_{$renderTo}', 'areaspline_{$renderTo}', 'scatter_{$renderTo}', 'pie_{$renderTo}'], function (i, type) {
     var res = type.split("_");
        $('#' + type).click(function () {
            {$renderTo}.series[0].update({
                type: res[0]
            });
        });
    });
