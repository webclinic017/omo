...
<div class="note note-success">
    <h4 class="block">Good news,</h4>
    <p>
        <b>We will send sms of execution report to mobile phone ( within every 5-10 minutes) if your order amount exceed 10,000 Tk. <br />Please update your mobile no if needed (call 8189296)</b>.
    </p>
</div>
        ...
        <div class="portlet light bg-inverse">
            <div class="portlet-title">
                <div class="caption font-green-sharp">
                    ...
                    <span class="caption-helper">buyer seller list (Automatically refreshed after 5 sec)</span>
                </div>
                <div class="tools">
...
<script>
    ...
    $(function () {
        ...
        var loanCode =<?php  if(isset($userListInfo[0]['User']['loanCode']))  echo $userListInfo[0]['User']['loanCode']; else echo 0;?>;
        var irn = '<?php echo $userListInfo[0]['User']['internal_ref_no']?>';
        ...
        $("#allShareList")
            .on("change", function () {
                ...
                var getNonMarginShare = '<?php echo Router::url('/', true)?>Instruments/checkMarginShare/' + instrumentId;
                $.ajax({
                    type: "POST",
                    dataType: 'json',
                    url: getNonMarginShare,
                    data: {instrumentId: instrumentId}

                }).done(function (data) {

                    if(data==0)
                    // toastr.error("This is margin share");
                        margin=0;
                    if(data==1)
                    //toastr.error("This is non margin share");
                        margin=1;
                });
        ...
        $('.demo-loading-btn')
            .click(function () {
                ...
                if ($("#Form1").valid()) {
                    ...
                    if(loanCode && margin){
                        toastr.error("Your code "+irn+" is loan code, and you are not permitted to buy/sell "+instrumentName+" non margin share");
        ...
        /* =================================call infinte ajax call -end======================================== */

        function fetch_order() {

            //////////////////////////buyer seller list auto change////////
            var orderType = $("#buyorsell").selectpicker("val");
            ...
            if(instrumentId!=0) {
                ...
                $.ajax({
                    ...
                    $("#market_depth2").html(data);
                    $('#dse').attr("data-url", getDseMarketDepthUrl);
                  //  toastr.success("DSE buy sell list changed");
                ...
                $.ajax({
                    ...
                    $("#market_depth_cse2").html(data);
                    $('#dse').attr("data-url", getCseMarketDepthUrl);
                  //  toastr.success("CSE buy sell list changed");
