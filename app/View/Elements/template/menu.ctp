...
$broker_id_show=$this->Session->read('Auth.User.broker_id');
...
foreach ($menu as $main_menu) {
    ...
    if(in_array($group_id, $main_menu['allowed_group'])) {
		//if($group_id==3) {
			if(isset($main_menu['allowed_house'])){
				 if(!in_array($broker_id_show, $main_menu['allowed_house'])) {
					continue;
				 }
        ...
        ?>
        <li id='<?php echo $id; ?>'>
            <a <?php echo $ajax_class; ?>
                href="<?php echo Router::url('/', true); ?><?php echo $main_menu['url']['controller']; ?>/<?php echo $main_menu['url']['action']; ?>">
            ...
            if (isset($main_menu['submenu'])) {
                ...
                <ul class="sub-menu">
                    <?php foreach ($main_menu['submenu'] as $submenu) {
                        ...
                        <li id='<?php echo $sid; ?>'>
                            <a <?php echo $ajax_class; ?>
                                href="<?php echo Router::url('/', true); ?><?php echo $submenu['url']['controller']; ?>/<?php echo $submenu['url']['action']; ?>">
